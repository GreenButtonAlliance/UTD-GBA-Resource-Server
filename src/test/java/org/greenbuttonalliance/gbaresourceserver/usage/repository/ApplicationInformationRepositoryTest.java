...
public class ApplicationInformationRepositoryTest {
	...
	@Test
	@DisplayName("ApplicationInformation request with clientId returns matching object")
	public void findByClientId_returnsCorrectApplicationInformation() {
		// Use the specific self-link HREFs to find the UUIDs
		UUID presentUuid1 = UuidCreator.getNameBasedSha1(UuidCreator.NAMESPACE_URL,
			"https://localhost:8080/DataCustodian/espi/1_1/resource/ApplicationInformation/135497");
		UUID foundUuid1 = applicationInformationRepository.findByClientId("GBA").map(ApplicationInformation::getUuid)
			.orElse(null);
		Assertions.assertEquals(presentUuid1, foundUuid1);
		...
		UUID presentUuid2 = UuidCreator.getNameBasedSha1(UuidCreator.NAMESPACE_URL,
			"https://localhost:8080/DataCustodian/espi/1_1/resource/ApplicationInformation/284915");
		UUID foundUuid2 = applicationInformationRepository.findByClientId("UtilityAPI")
			.map(ApplicationInformation::getUuid).orElse(null);
		Assertions.assertEquals(presentUuid2, foundUuid2);
	}

	@Test
	@DisplayName("ApplicationInformation request with  non-existing clientId returns empty Optional")
	void findByClientId_nonExisting_returnsEmpty() {
		// Generate a clientId that is guaranteed not to exist in the database
		String nonExistingClientId = "NonExistingClientId";

		// Attempt to find an ApplicationInformation by the non-existing clientId
		Optional<ApplicationInformation> result = applicationInformationRepository.findByClientId(nonExistingClientId);

		// Assert that the result is empty
		Assertions.assertTrue(result.isEmpty(), "Expected an empty Optional when querying a non-existing clientId");
	}

	@Test
	@DisplayName("ApplicationInformation request with known dataCustodianId returns all relevant " +
		"ApplicationInformation objects")
	void findAllByDataCustodianId_returnsAllRelevantApplicationInformation() {
		// Generate a UUID that is guaranteed not to exist in the database
		String dataCustodianId = "GBA Electricity & Gas";

		// Attempt to find an ApplicationInformation by the non-existing UUID
		Optional<ApplicationInformation> result =
			applicationInformationRepository.findByDataCustodianId(dataCustodianId);

		// Assert that the result is present and the dataCustodianId matches
		Assertions.assertTrue(result.isPresent(),
			"Expected a non-empty Optional when querying an existing dataCustodianId");
		Assertions.assertEquals(dataCustodianId, result.get().getDataCustodianId(),
			"The dataCustodianId should match the queried value");
	}

	@Test
	@DisplayName("ApplicationInformation request with  non-existing clientId returns empty Optional")
	void findByDataCustodianId_nonExisting_returnsEmpty() {
		// Generate a clientId that is guaranteed not to exist in the database
		String nonExistingDataCustodianId = "NonExistingClientId";

		// Attempt to find an ApplicationInformation by the non-existing clientId
		Optional<ApplicationInformation> result = applicationInformationRepository
			.findByClientId(nonExistingDataCustodianId);

		// Assert that the result is empty
		Assertions.assertTrue(result.isEmpty(),
			"Expected an empty Optional when querying a non-existing clientId");
	}
}
